...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Calma ae lek, estou fazendo ⌛',
				success: 'Pronto ✔️',
				error: {
			...
			colors = ['vermelho ',' branco ',' preto ',' azul ',' amarelo ',' verde']
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*Nome do bot* : ${meu nome}\n*Número do bot* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Total de contatos bloqueados* : ${blocked.length}\n*The bot ativo desde* : ${kyun(uptime)}`
				...
				case 'gtts':
					...
					break
				case 'meme':
					meme = await fetchJson('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS3hVs4ajbl4FPAJpgllg8Vx-n_b9XAVTFnoQ&usqp=CAU', { method: 'get' })
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSMIGFWO7TG2xvHYaCQ_ZOe060h_RC9kFqkAA&usqp=CAU`)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Ta querendo adicionar mais um corno ??')
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('📌 cade o sticker para eu fazer')
					reply(mess.wait)
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Já ativo um minuto')
						welkom.push(from)
